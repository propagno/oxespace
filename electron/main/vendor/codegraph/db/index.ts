...
import { assetPath } from '../asset-dir';
...
export class DatabaseConnection {
  ...
  static initialize(dbPath: string): DatabaseConnection {
    ...
    const schemaPath = assetPath('schema.sql');
...
export function getDatabasePath(projectRoot: string): string {
  // Use the configured CodeGraph dir (CODEGRAPH_DIR = '.oxe' inside OXESpace) so
  // the DB lands where isInitialized()/open() look. Was hardcoded '.codegraph',
  // which caused init() to write to .codegraph/ while open() checked .oxe/ →
