...
VALUES
  ...
  ('builtin-agent-gemini', 'Gemini', 'gemini', 'gemini',       '{{task}}', NULL, NULL, 1, strftime('%s', 'now') * 1000),
...
UPDATE agent_profiles SET name = 'Gemini', is_builtin = 1 WHERE agent_profile_id = 'builtin-agent-gemini';
...
VALUES
  ...
  ('builtin-gemini', 'gemini', 'gemini',       '[]', 1),
  ('builtin-cursor', 'cursor', 'cursor-agent', '[]', 1);

DELETE FROM agent_readiness_cache
WHERE provider NOT IN ('claude', 'copilot', 'codex', 'gemini', 'cursor');
