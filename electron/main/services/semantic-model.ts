 ...
 *
 * `intfloat/multilingual-e5-small` (via Xenova ONNX): 384-dim, 512-token window,
 * ~100 languages — closes the cross-lingual gap (Portuguese queries against
 * English code) that the English-only all-MiniLM-L6-v2 left open, while being
 * ~2–3× faster and ~120MB (vs ~280MB for e5-base) so it can be bundled offline
 * for locked-down/corporate networks (see scripts/fetch-semantic-model.mjs).
 *
...
export const SEMANTIC_EMBED_DIM = 384
